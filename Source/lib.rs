...
pub fn with_store<R:Runtime, T, F:FnOnce(&mut Store<R>) -> Result<T, Error>>(
	...
	f(stores.get_mut(path).expect("failed to retrieve store. This is a bug!"))
}
...
async fn keys<R:Runtime>(
	...
	with_store(app, stores, path, |store| Ok(store.keys().cloned().collect()))
}
...
async fn values<R:Runtime>(
	...
	with_store(app, stores, path, |store| Ok(store.values().cloned().collect()))
}
...
async fn entries<R:Runtime>(
	...
	with_store(app, stores, path, |store| {
		Ok(store.entries().map(|(k, v)| (k.to_owned(), v.to_owned())).collect())
	})
}
...
impl<R:Runtime> Default for Builder<R> {
	fn default() -> Self { Self { stores:Default::default(), frozen:false } }
}
